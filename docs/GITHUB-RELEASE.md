...
4. Commit the intended release state and create the annotated `v0.5.0` tag.
5. Run the normal notarized installer. It creates and verifies the app, DMG, and
   checksum before installing anything globally.
6. Run `scripts/verify_release.sh 0.5.0`. It requires the clean, exact release
...
9. Create a draft GitHub release from the exact tag:
   ...
   gh release create v0.5.0 \
     Poor-Mans-Text-0.5.0.dmg \
     Poor-Mans-Text-0.5.0.dmg.sha256 \
     --verify-tag \
     --draft \
     --title "Poor Man's Text 0.5.0" \
     --notes-file docs/releases/0.5.0.md
   ```
