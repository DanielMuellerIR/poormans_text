...
| **Tech-Stack** | Swift 6, native macOS-Frameworks (AppKit, Foundation, PDFKit/Vision), Pandoc-Sandbox, CLI + SwiftUI-App. | Python 3.10+, Standard-Open-Source-Parser (`mammoth`, `pdfminer`, `openpyxl`, `python-pptx`), LLM-Clients. |
...
| **PDF** | **PDF-Dateien** | PDFKit-Text pro Seite, lokaler Vision-OCR nur für textarme Seiten, Seiten- und Pixelbudgets sowie sichtbare Layoutwarnung. | `pdfminer.six` + `pdfplumber`: Extrahiert Text und Tabellenlayout rein in Python. | **MarkItDown** bietet Tabellenlayout; **Poor Man's Text** isoliert die native, lokale Inhaltsübernahme strenger. |
| **Bilder & OCR** | **Bilder (PNG/JPG/TIFF)** | Native ImageIO-Übernahme für PNG, JPEG, HEIC und TIFF als unveränderte Assets; optionale lokale Vision-OCR mit Pixelbudgets. | EXIF via ExifTool + OCR / Bildbeschreibung via LLM (OpenAI GPT-4o Vision). | **MarkItDown** nutzt multimodale KI; **Poor Man's Text** fokussiert Offline-Datenschutz & System-OCR. |
